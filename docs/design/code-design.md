...
`loopCap` (the max Decide iterations per task). Token budgets, the loop cap,
and temperature are deliberately *not* constants in code: a 1B model on a Pi
and a 200B model on a workstation want very different values (user story 18).
